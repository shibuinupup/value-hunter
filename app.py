...
# ---- 0x quote cache (TTL) + rate limit backoff ----
# Cache key includes taker because allowance-holder routing can depend on taker.
ZEROX_CACHE_TTL_S = float(os.getenv("ZEROX_CACHE_TTL_S", "45"))
ZEROX_MAX_RETRIES_429 = int(os.getenv("ZEROX_MAX_RETRIES_429", "5"))
ZEROX_BACKOFF_BASE_S = float(os.getenv("ZEROX_BACKOFF_BASE_S", "0.6"))
ZEROX_BACKOFF_CAP_S = float(os.getenv("ZEROX_BACKOFF_CAP_S", "8.0"))

# key -> (ts, json_or_none)
_zero_x_cache: dict[tuple, tuple[float, dict | None]] = {}
_zero_x_cache_lock = threading.Lock()

def _cache_get(key):
    now = time.time()
    with _zero_x_cache_lock:
        item = _zero_x_cache.get(key)
        if not item:
            return None, False
        ts, val = item
        if now - ts > ZEROX_CACHE_TTL_S:
            _zero_x_cache.pop(key, None)
            return None, False
        return val, True
...
def _cache_set(key, val):
    now = time.time()
    with _zero_x_cache_lock:
        _zero_x_cache[key] = (now, val)

def zerox_price(chain_id: int, taker: str | None, sell_token: str, sell_amount_raw: int, buy_token_addr: str):
    params = {
    ...
    # taker is optional for /price (v2). Only include if it's a real address.
    if taker and taker.lower() != "0x0000000000000000000000000000000000000000":
        params["taker"] = taker
    ...
    last_429_text = None
    for attempt in range(max(1, ZEROX_MAX_RETRIES_429)):
        try:
        ...
        if r.status_code == 429:
            last_429_text = (r.text or "")[:250]
            # exponential backoff + jitter
            wait = min(ZEROX_BACKOFF_CAP_S, ZEROX_BACKOFF_BASE_S * (2 ** attempt)) + random.random() * 0.25
            print("0x 429 rate limited; backoff", round(wait, 2), "s")
            time.sleep(wait)
            continue
    ...
    # After retries
    if last_429_text is not None:
        print("0x error 429 (exhausted retries)", "chain", chain_id, "sell", sell_token, "buy", buy_token_addr, "body", last_429_text)
    _cache_set(cache_key, None)
    return None
...
def debug_quote():
    # sanity test: WETH -> USDC should route on Ethereum (unless rate-limited)
    chain_id = 1
    weth = CHAIN_CONFIG[1]["wrapped"]
    usdc = CHAIN_CONFIG[1]["stable"]
    taker = os.getenv("DEBUG_TAKER")  # optional; set to real address
    sell_amount = 10**15  # 0.001 WETH
    q = zerox_price(chain_id, taker if taker else None, weth["address"], sell_amount, usdc["address"])
